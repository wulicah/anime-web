...
总共 6 个核心页面 + 季度归档子页，按使用路径排序：

1. **首页 (Home)**：当前季度新番表（按星期分组），本周即将开播番剧高亮。
2. **番剧详情页 (Detail)**：番剧信息（封面、简介、声优、staff、平台）、用户评分分布、个人追番按钮、剧集列表、评论。
3. **我的追番页 (Library)**：分"在看 / 想看 / 看过 / 弃番"四状态，展示个人评分与更新时间线。
4. **搜索页 (Search)**：按番剧名 / 声优 / tag 搜索，结果可筛选。
5. **归档页 (Archive)**：历年春/夏季新番列表（致敬 yuc.wiki 的 archives），可按年份翻页。
6. **季度归档页 (ArchiveSeason)**：按季度浏览番剧列表，增量加载。
...
| 首页 | 卡片网格 | 移动端 1 列、平板 2 列、桌面 3-4 列。 |
